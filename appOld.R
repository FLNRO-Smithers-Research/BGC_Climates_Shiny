    ...
    tabPanel("Two Variable Summary",
             ...
             column(6,
                    ...
                    pickerInput("compNormPer",
                                "Select Normal Period:",
                                choices = period.choose,
                                inline = FALSE,
                                multiple = FALSE,
                                selected = "1961 - 1990")
                    ),
...
server <- function(input, output) {
  ...
  output$twovar <- renderPlot({
    if(!is.null(input$znPick)){
      dat <- getCompDat()
      tZone <- t(colMeans(dat[,-(1:3)])) %>% as.data.frame()
      browser()
      
      ggplot(dat, aes(x = get(input$xvar), y = get(input$yvar))) +
        geom_label(aes(label = bgc,colour = bgc)) +
        geom_point(data = tZone, aes(x = get(input$xvar), y = get(input$yvar)),size = 6, colour = "black")+
    ...
    local({
      ...
      output[[plotname]] <- renderPlot({
        ...
        if(input$grType == "Bar" | input$grType == "Line"){
          graph <- data[data$ClimateVar == selectVars[my_i] & 
...
################Station data######################################
  ...
  stPrepGraph <- reactive({
    ...
    modelSub$Type <- "Model"
    m.oldSub <- dbGetQuery(con, paste0("SELECT ",paste(c("station", input$var.pick),collapse = ","),
                                       " FROM stpoints_mod61 WHERE station IN ('",
                                       paste(input$stn.pick,collapse = "','"),"')"))
    m.oldSub$Type <- "Model61-90"
    stationSub <- dbGetQuery(con, paste0("SELECT ",paste(c("station", input$var.pick),collapse = ","),
    ...
    local({
      ...
      output[[plotname]] <- renderPlot({
        ...
        if(length(input$stn.pick) > 0 & length(input$var.pick) > 0){
          dat <- stPrepGraph()
          dat <- dat[,c("station","Type",input$var.pick[my_j])] %>% set_colnames(c("Station","Type","Mean"))
